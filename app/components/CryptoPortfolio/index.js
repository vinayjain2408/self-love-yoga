...
const CryptoPortfolio = () => {
  ...
  return (
    <div className="min-h-screen bg-green-800 flex flex-col items-center justify-center text-white px-4 py-10">
      ...
      {/* Category Tabs */}
      <div className="flex gap-3 mb-8 bg-green-700 rounded-full px-2 py-2">
        {categories.map((cat) => (
          <button
            key={cat}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 max-w-6xl mx-auto mt-10">
        {cryptos.map((coin) => (
          <div
            key={coin.symbol}
            className="bg-green-700 hover:bg-green-600 rounded-xl px-4 py-2 shadow-md transition-all border border-white flex items-center space-x-3"
          >
            <img
